...
public:
    ...
    static std::vector<unsigned char> processScalar(
        ...
        for (size_t i = 0; i < size; ++i) {
            float val = static_cast<float>(input[i]) * inv_255;
            val *= scale;
    ...
    static std::vector<unsigned char> processSIMD(
        ...
        for (; i + 7 < size; i += 8) {
            // Load 8 bytes
            float temp_in[8];
            for(int k=0; k<8; ++k) temp_in[k] = static_cast<float>(input[i+k]);
            __m256 v_data = _mm256_loadu_ps(temp_in);
            ...
            int32_t temp_out[8];
            _mm256_storeu_si256(reinterpret_cast<__m256i*>(temp_out), v_int);
            for(int k=0; k<8; ++k) output[i+k] = static_cast<unsigned char>(temp_out[k]);
        }
#endif
        // Remainder
        for (; i < size; ++i) {
    ...
    static BenchmarkResult runBenchmark(
        ...
        for (int i = 0; i < iterations; ++i) {
            auto result = process_func(input, size, scale);
        }
...
int main() {
    ...
    for (const auto& [size, description] : test_sizes) {
        ...
        std::cout << "  Speedup: " << (scalar_result.time_ms / simd_result.time_ms) << "x" << std::endl;
